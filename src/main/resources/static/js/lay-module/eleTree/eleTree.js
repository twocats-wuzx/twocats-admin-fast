...
layui.define(["jquery","laytpl"], function (exports) {
    ...
    const cssUrl = layui.cache.modules["eleTree"].replace('.js', '.css');
    if (cssUrl)
        layui.link(cssUrl);
    ...
    // 模板渲染
    const TPL_ELEM = function (options, floor, parentStatus) {
        return [
...
// 是否显示连线
    ...
    Class.prototype={
        ...
        customKeyInit: function() {
            const options = this.config;
            let key = "";
            for(let i=0; i<options.request.key.length; i++){
                const s = options.request.key.charAt(i);
        ...
        eleTreeEvent: function() {
            ...
            const options = this.config;
            // 下拉
            const expandOnClickNode = options.expandOnClickNode ? ".eleTree-node-content" : ".eleTree-node-content>.eleTree-node-content-icon";
            options.elem.on("click",expandOnClickNode,function(e) {
                ...
                if(sibNode.children(".eleTree-node").length===0){
                    const floor = Number(eleNode.attr("eletree-floor")) + 1;

                    // 选择祖父
                    const selectParentsFn = function () {
                        if (!options.checkStrictly) {
                    ...
                    const data = _self.reInitData(eleNode);
                    ...
                    if(options.lazy && el.hasClass("lazy-icon")){
                        ...
                        const eletreeStatus = eleTreeNodeContent.children("input.eleTree-hideen").attr("eletree-status");
                        if(d[options.request.children] && d[options.request.children].length>0){
                ...
                if(options.accordion){
                    const node = eleTreeNodeContent.parent(".eleTree-node").siblings(".eleTree-node");
